function [ output_args ] = find_change_DA(  )
...
for i = 1:length(file)
    ...
    psth_da = [psth_da;temp.psth_mean];
    psth_max=[psth_max;max(temp.psth_mean(1,pre_stim/bin:(pre_stim+xp2)/bin))];
        %%%%%%calculate area under curve (AUC)
    temp1 = aucCalculate2(x,win1,temp.psth_mean);
    temp2 = aucCalculate2(x,win2,temp.psth_mean);
    temp3 = aucCalculate2(x,win3,temp.psth_mean);
    group_temp = [temp1 temp2 temp3];
    psth_auc = [psth_auc;group_temp];
end
...
assignin('base','group_statistic',group_statistic);
