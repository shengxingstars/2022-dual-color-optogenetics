function[coeff,denCTrial,denCoeff,yDen]=Run_NZT(nx,ncDiff,sc)
zeroIdx= find(nx == 0);
[h,g,rh,rg] = load_filter_coefficients;
lav=length(ncDiff);lh=length(h);lg=length(g);lrh=length(rh);lrg=length(rg);
yDen=zeros(sc+1,lav);
coeff=zeros(sc+1,lav);
denCoeff=zeros(sc+1,lav);
thresh=[]; den_coeff_Neigh={}; denoised_coeff={}; original_coeff={};
%% Convolution
th=ncDiff;
for s=1:sc
    tga=zeros(1,length(th));tha=zeros(1,length(th));
    for k=1:length(th);                   
        for i=-lh/2:lh/2-1;
            if i>-k;if i<length(th)-k;
                    tha(k) = tha(k) + th(i+k) * h(i+lh/2+1);
            end; end
        end 
        for i=-lg/2:lg/2-1;
            if i>-k;if i<length(th)-k;
                    tga(k) = tga(k) + th(i+k) * g(i+lg/2+1);
            end; end
        end 
    end
    tg=tga; th=tha;
    tg=tg(1:2:length(tg));                 
    th=th(1:2:length(th));
    original_coeff{s}=tg;
    original_coeff{sc+1}=th;    
%% denoising details
    L_tg=length(tg);
%     tgm=tg(1:L_tg/2)-mean(tg(1:L_tg/2));
    Pow=nextpow2(zeroIdx-1);
    L_baseline=2^Pow;
    baseEnd = L_baseline/(2^s);
    if baseEnd > 2
        tgm=tg(1:baseEnd)-mean(tg(1:baseEnd));
        bl_var=median(abs(tgm)/0.6745);   
        LS = [tg(2:length(tg)) tg(1)];
        RS= [tg(length(tg)) tg(1: length(tg)-1)];
        e_tg=RS.^2+tg.^2+LS.^2;
        thresh(s) =(sqrt(2*(bl_var^2)* log(L_tg)))^2; 
        e_tg( find( abs(e_tg)<=abs(thresh(s)) ) ) = 0;
        mask=e_tg & ones(1,L_tg);
        tg=mask.*tg;      
    end
        den_coeff_Neigh{s}=tg;  
end
%% denoising the last approximation
if baseEnd > 2
    L_th=length(th);
%     thm=th(1:L_th/2)-mean(th(1:L_th/2));
    thm=th(1:baseEnd)-mean(th(1:baseEnd));
    bl_var=median(abs(thm)/0.6745);
    LS = [th(2:length(th)) th(1)];
	RS= [th(length(th)) th(1: length(th)-1)];
    e_th=RS.^2+th.^2+LS.^2;        
    thresh(sc+1) =(sqrt(2*(bl_var^2)* log(L_th)))^2;       
    e_th( find( abs(e_th)<=abs(thresh(sc+1)) ) ) = 0;
    mask=e_th & ones(1,L_th);
    th=mask.*th;
end
den_coeff_Neigh{sc+1}=th;
%% ZeroTrees denoising
denoised_coeff{sc+1}=den_coeff_Neigh{sc+1};
d_coeff=den_coeff_Neigh{sc+1};
mask=d_coeff& ones(1,length(d_coeff));
d_coeff=den_coeff_Neigh{sc};
n_coeff=[];
for j=1:length(d_coeff)
     if mask(j)==0                            
              n_coeff(j)=0;
     else
         n_coeff(j)=d_coeff(j);
    end
end
    denoised_coeff{sc}=n_coeff;
for i=sc:-1:2  
    d_coeff=denoised_coeff{i};
    mask=d_coeff& ones(1,length(d_coeff));
    mask=[mask;mask];
    mask=mask(:)';
    d_coeff=den_coeff_Neigh{i-1};
    n_coeff=[];
    for j=1:length(mask)
        if mask(j)==0                           
           n_coeff(j)=0;  
        else
            n_coeff(j)=d_coeff(j);
        end
    end
    denoised_coeff{i-1}=n_coeff;
end
%% reconstruction and fill with 0s
 for s=1:sc
    TG=original_coeff{s};
    tg=denoised_coeff{s}; 
    for j=s:-1:1                          
        if j==s
            TG=[TG;zeros(1,length(TG))];
            TG=TG(:)';
            tg=[tg;zeros(1,length(tg))];
            tg=tg(:)';
            
            tm=tg;
            tma=zeros(1,length(tm));
            for k=1:length(tm);
                for i=-lrg/2+1:lrg/2;
                    if i>-k;if i<length(tm)-k;
                            tma(k) = tma(k) + tm(i+k) * rg(i+lrg/2);
                    end; end
                end 
            end
            tm = tma;
        else
            TG=[TG;zeros(1,length(TG))];
            TG=TG(:)';
            tg=[tg;zeros(1,length(tg))];
            tg=tg(:)';
            tm=[tm;zeros(1,length(tm))];
            tm=tm(:)';
            tma=zeros(1,length(tm));
            for k=1:length(tm);
                for i=-lrh/2+1:lrh/2;
                    if i>-k;if i<length(tm)-k;
                            tma(k) = tma(k) + tm(i+k) * rh(i+lrh/2);
                    end; end
                end 
            end
            tm = tma;
        end
    end
   yDen(s,:)=tm;
   coeff(s,:)=TG;
   denCoeff(s,:)=tg;
 end
%% reconstruction of the last approximation
    TH=original_coeff{sc+1};
    th=denoised_coeff{sc+1};
    ta=th;
    for j=1:sc 
        TH=[TH;zeros(1,length(TH))];
        TH=TH(:)';
        th=[th;zeros(1,length(th))];
        th=th(:)';
        ta=[ta;zeros(1,length(ta))];
        ta=ta(:)';
        taa=zeros(1,length(ta));
        for k=1:length(ta);
            for i=-lrh/2+1:lrh/2;
                   if i>-k;if i<length(ta)-k;
                             taa(k) = taa(k) + ta(i+k) * rh(i+lrh/2);
                   end; end
            end
        end
    ta = taa;
    end
yDen(sc+1,:)=ta;
coeff(sc+1,:)=TH;
denCoeff(sc+1,:)=th;
denCTrial=sum(yDen);
